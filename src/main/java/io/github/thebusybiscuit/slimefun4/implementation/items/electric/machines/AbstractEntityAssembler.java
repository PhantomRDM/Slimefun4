...
public abstract class AbstractEntityAssembler<T extends Entity> extends SimpleSlimefunItem<BlockTicker> implements EnergyNetComponent {
    ...
    private void updateBlockInventory(BlockMenu menu, Block b) {
        if (!BlockStorage.hasBlockInfo(b) || BlockStorage.getLocationInfo(b.getLocation(), KEY_ENABLED) == null || BlockStorage.getLocationInfo(b.getLocation(), KEY_ENABLED).equals(String.valueOf(false))) {
            menu.replaceExistingItem(22, new CustomItem(Material.GUNPOWDER, "&7啟用d: &4\u2718", "", "&e> 點擊啟動機器"));
            ...
            menu.replaceExistingItem(22, new CustomItem(Material.REDSTONE, "&7啟用: &2\u2714", "", "&e> 點擊關閉機器"));
            menu.addMenuClickHandler(22, (p, slot, item, action) -> {
        ...
        menu.replaceExistingItem(31, new CustomItem(Material.PISTON, "&7生成位置(上方): &3" + offset + " 格", "", "&f左鍵: &7+0.1", "&f右鍵: &7-0.1"));
    ...
    protected void constructMenu(BlockMenuPreset preset) {
        preset.addItem(1, new CustomItem(getHead(), "&7頭顱槽位", "", "&f此插槽僅接受頭顱類型"), ChestMenuUtils.getEmptyClickHandler());
        preset.addItem(7, new CustomItem(getBody(), "&7身體槽位", "", "&f此插槽僅接受身體類型"), ChestMenuUtils.getEmptyClickHandler());
        preset.addItem(13, new CustomItem(Material.CLOCK, "&7冷卻: &b30 秒", "", "&f這台機器需要30秒的時間才能操作", "&f給它一些時間！"), ChestMenuUtils.getEmptyClickHandler());
    }
